...
Forwards meet the interactive workflow: previously a host's port-forwards
were only reachable via `xzssh tunnel` or a generated `~/.ssh/config` —
`xzssh connect` never opened them, and the TUI had no path to tunnels at
all.

### Added

- **`connect --forwards`** opens the host's LocalForward / RemoteForward /
  DynamicForward rules with the session, as `-L`/`-R`/`-D` flags on the
  ssh command line. Plain `connect <alias>` is unchanged (no forwards, no
  prompt), so scripts keep working; `--dry-run` reflects the flag.
- **Forward prompt in the interactive picker.** When `connect` is used
  without an alias (the fuzzy picker — including from the welcome menu)
  and the chosen host has forwards, xzSSH asks once whether to open them
  with the session. Answering no (or dismissing) gives a plain connect.
  A busy local port can't break the session: ssh warns and continues.
- **`(t) Tunnels` in the management menu** (`xzssh menu` / `m` from the
  welcome screen): start a tunnel (picks among forward-bearing hosts,
  foreground or detached), list recorded tunnels, and stop one — the
  `tunnel` subcommand's start/list/stop, now discoverable from the TUI.

### Changed

- `build_ssh_command` grew an `include_forwards` parameter;
...
wrappers — they reuse a host's resolved connection settings so you don't
retype them.

### Added

- **`xzssh known-hosts remove <alias>`** — wraps `ssh-keygen -R` to drop
  a host's cached key after a server rebuild (the
  `REMOTE HOST IDENTIFICATION HAS CHANGED` wall). Brackets the
  `[host]:port` form for non-default ports automatically and honours the
  host's `UserKnownHostsFile`. `--dry-run` prints the command.
- **`xzssh mosh <alias>`** — connect with [mosh](https://mosh.org/)
  using the host's port / identity / ProxyJump / `-o` options (handed to
  mosh via `--ssh="ssh …"`, reusing `build_ssh_command`). `--dry-run`
  prints the command. Unlike `connect`, it doesn't stamp `last_used` or
  write the event log.

## [0.22.0] — 2026-06-29

Host-management convenience — both edit the JSON store through the usual
`write_config` path, no `edit`/`$EDITOR` round-trip.

### Added

- **`xzssh tag add <alias> <tag>…`** and **`xzssh tag rm <alias>
  <tag>…`** — manage a host's tags directly. `add` is idempotent (a tag
  the host already has is reported, not duplicated); `rm` reports any
  tag the host didn't have and otherwise succeeds. `tag rm` completes
  the host's existing tags.
- **`xzssh add --from <alias>`** — clone an existing host as a template
  for a new one. Every field is copied from the source; any explicit
  flag overrides the copied value; `last_used` is never copied. Requires
  a new `--alias`.
...
- **Native zsh completion** ([`completions/_xzssh`](completions/_xzssh)).
  Groups commands, sub-commands, and options into separate labelled
  sections (instead of argcomplete's single flat list), and completes
  live host aliases, registered keys, and profiles from the active
  config. Install it on `$fpath` and `compdef _xzssh xzssh`; see the
  README. argcomplete remains the option for bash/fish.

### Changed

- **Friendly errors for a mistyped sub-command.** `xzssh lsit` now
  prints a themed `Unknown command: lsit / Did you mean list?` (exit 2)
  instead of argparse's raw usage-block dump, with `difflib`-based
  suggestions that also work for nested sub-commands (`xzssh key gne` →
  `gen`). Invalid *option* values keep argparse's default error so their
  valid choices still show.

## [0.21.0] — 2026-06-20

First item off the post-roadmap backlog after the v0.20.x key/fix work:
an escape hatch so the model no longer needs a code change per ssh
directive.

### Added

- **Free-form `options` passthrough on a host.** Each host gains an
  ordered `options` map of arbitrary `ssh_config` directives —
  `xzssh add --option ControlMaster=auto --option 'SetEnv=FOO=bar'` (or
  edit the JSON / `xzssh edit`). They render verbatim in the generated
  config and are injected as `-o Key=value` into `connect` / `which` /
  `test`, so directives xzSSH doesn't model first-class (ControlMaster,
  ControlPersist, RemoteCommand, SetEnv, Ciphers, AddKeysToAgent, …)
  work without waiting for a dedicated field. `--option` splits on the
  first `=` only, so values may contain `=`.
- The typed fields still render first, so an option naming a directive
  xzSSH already manages (e.g. `Port`) is shadowed — the validator
  **warns** about that rather than silently dropping it.

### Notes

- `options` are written by `generate` and honored by the interactive
  commands, but are **not** yet captured by `import` or compared by
  `sync` (the OpenSSH parser doesn't model arbitrary directives, so
  comparing would report spurious drift). Round-tripping them through
  import is a possible follow-up.
...
[Unreleased]: https://github.com/010228lxz/xzSSH/compare/v0.24.0...HEAD
[0.24.0]: https://github.com/010228lxz/xzSSH/compare/v0.23.1...v0.24.0
[0.23.1]: https://github.com/010228lxz/xzSSH/compare/v0.23.0...v0.23.1
[0.23.0]: https://github.com/010228lxz/xzSSH/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/010228lxz/xzSSH/compare/v0.21.1...v0.22.0
[0.21.1]: https://github.com/010228lxz/xzSSH/compare/v0.21.0...v0.21.1
[0.21.0]: https://github.com/010228lxz/xzSSH/compare/v0.20.1...v0.21.0
